import type { GateDecision, ScoreGate } from '../contracts/index.js'
...
/**
 * The grounding floor: top fused score strictly below this -> `band: 'refuse'`.
 *
 * ⚠️ CALIBRATE (GAP-3, FTR-31 SC-7): the magnitude depends on the RRF `fused`
 * score SCALE owned by the `retrieval` timeline (ADR-003, k=60). This is a
 * documented placeholder; calibrate it against real fused-score output in the
 * gated cost dogfood. The test suite asserts the BRANCH (below -> refuse,
 * at/above -> answer), not this number, so recalibration is a one-line change.
 * Must stay > 0 so an empty/zero-grounding query can refuse.
 */
export const GROUNDING_FLOOR = 0.015
...
export const scoreGate: ScoreGate = (retrieval, query) => {
  ...
  // Top result's fused score is the strongest, most interpretable grounding
  // signal (a mean would dilute a single strong hit). Empty retrieval -> 0.
  const groundingScore = retrieval[0]?.fused ?? 0
